...
export const MisProductos = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api_product/product/${id}`, {
        method: "DELETE",
        headers,
        body: token ? undefined : JSON.stringify({ user_id: currentUser?.id }),
      });
  ...
  return (
    <div className="container my-4">
      ...
      {items.length === 0 ? (
        <div className="alert alert-info">Aún no has publicado productos.</div>
      ) : (
        <div className="row g-3">
          {items.map((p) => {
            const imgArray = imagesByCategory[normalizeCategory(p.category)] || [image_not_found];
            // Usa la primera imagen de la categoría o fallback
            const mainImage = imgArray[0] || image_not_found;
            return (
              <div className="col-12 col-sm-6 col-md-4 col-lg-3" key={p.id}>
                <div className="card h-100">
                  <img src={mainImage} className="card-img-top" alt={p.title} />
